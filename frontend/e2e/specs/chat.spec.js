...
test.describe('Chat flow', () => {
    ...
    test('receives a visualization for a data question', async ({ page }) => {
        ...
        await expect(page.locator('bi-bar-chart, bi-data-table, bi-metric, bi-narrative-text').first()).toBeVisible({ timeout: 90_000 });
    });
...
test.describe('Chat SSE events', () => {
    test('receives SSE events from backend', async ({ page }) => {
        ...
        if (hasSpec)
            console.log('SpecFragmentEvent received');
    });
});
